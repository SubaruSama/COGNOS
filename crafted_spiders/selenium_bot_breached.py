...
def extract_contents_from_posts(html_content: str) -> str:
    ...
    xpath_title = browser.find_element(
        By.XPATH, "/html/body/div[1]/main/table[1]/tbody/tr[1]/td/div/span"
    )
    ...
    #             By.XPATH, '//*[@id="posts"]/div[*]/div[1]/div[1]/div[1]/div[1]/a/span'
    #         )
    #         item.username = [username.text for username in usernames]

    #         info_date_posts = post.find_elements(
    #             By.XPATH, '//*[@id="posts"]/div[*]/div[2]/div[1]/div[1]/span'
    #         )
    ...
    #         logger.debug(f"Item contents: {item}")
    ...
    #     finally:
    #         logger.debug(f"{asdict(item)}")
    #         write_to_file(asdict(it))
...
def save_to_json(
    file_name: str, file_extension: str, item: dict[str, any], file_path: str = None
) -> None:

    # raise NotImplementedError("Changing some things on code login. This functions NEED to be reviewed.")

    logger.debug("Creating the json file of scraped data...")
    if file_path is None:
        outfile = open(f"{file_name}{file_extension}", mode="a+", encoding="utf8")
    else:
        if file_path[-1] == "/":
            file_path = file_path[:-1]  # Dirty hack
        outfile = open(
            f"{file_path}/{file_name}{file_extension}", mode="a+", encoding="utf8"
        )
    logger.debug(f"Created json of results: {outfile.name}")
    logger.debug(f"Path of the json: {os.path.realpath(outfile.name)}")

    try:
        json.dump(f"{item},", outfile, indent=4)
...
while True:
    try:
        ...
        go_to_page(
            urls.get("breached_hidden_service_base", "URL not registered") + "/search"
        )
        make_search()
        html_content = selenium_page_source(browser)
